...
- `standards/backend/entities.md` - Entity naming and definition patterns
- `standards/backend/status-workflow.md` - Status workflow patterns

**Framework Guide Reference**: For advanced topics, see `runtime/component/moqui-agent-os/framework-guide.md`:
- **"## Mantle UDM Overview"** - Business model concepts and entity relationships
...
Invoice: fromPartyId (seller) → toPartyId (buyer) with InvoiceItems. Types: Sales/Receivable (InvoiceSales), Purchase/Payable (InvoicePurchase), Return (InvCustReturnCredit/InvRtnShipReplace).
...
5. VENDOR INVOICE
   Create Invoice (invoiceTypeEnumId="InvoicePurchase")
