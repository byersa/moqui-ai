...
async function verifyClientToken(info, callback) {
    ...
    // Special case for MCP path - allow all local connections for now
    if (path === MCP_PATH) {
        console.error(`MCP connection attempt on path: ${path}`);
        return callback(true);
    }

    // Special case for registration path - allow always (auth happens in message handler)
...
wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (message) => {
        ...
        try {
            // Handle simple non-JSON heartbeats
            if (rawMessage === 'ping' || rawMessage.trim() === 'ping') {
                console.error(`Received simple heartbeat on ${clientChannel}`);
            ...
            console.error(`Error processing WebSocket message from ${clientChannel}: ${error.message}. Processed message was: [ ${message.toString()} ]`);
            try {
                ws.send(JSON.stringify({
                    type: 'error',
...
function handleRegisterTool(ws, channelPath, data) {
    ...
    console.error(`Registering tool ${name} on channel ${channelPath}`);

    // Sanitize input schema
    const safeSchema = (inputSchema && inputSchema.type === 'object') ? inputSchema : {
        type: 'object',
        properties: {}
    };
...
function handleListTools(ws, clientChannel, data) {
    ...
    if (isMcpClient) {
        ...
        tools = Object.entries(toolsRegistry).map(([toolId, toolInfo]) => {
            ...
            return {
                name: pathBasedName,
                description: toolInfo.description || `Tool: ${toolInfo.originalName}`,
                inputSchema: (toolInfo.inputSchema && toolInfo.inputSchema.type === 'object') ? toolInfo.inputSchema : {
                    type: 'object',
                    properties: {}
                },
            };
...
function handleToolResponse(data) {
    ...
    console.error(`Received tool response for original ID ${originalId} (internal ID ${id}): result=${JSON.stringify(result)}, error=${error}`);
